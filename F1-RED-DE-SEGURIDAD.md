...
- **`npm test`** encadena: `qa:rundown-order` (unit) → `qa:templates` (auditoría
  estática de los 16 estilos únicos, uno por plantilla) →
...
2. `npm test` — la primera pasada renderiza todo (más lenta) y **aprueba la
   ...
   La referencia visual ya no recorre combinaciones de color innecesarias.
...
Esta cifra corresponde a la entrega histórica F1, antes de retirar las
paletas: unit OK · humo 9/9 (34,7 s) · matriz visual 0,000%. Nota honesta
(regla 4): el sandbox corta cada comando a 45 s,
