...
module.exports = {
  ...
  build(card, ctx) {
    ...
    if (hasCaption) {
      ...
      const titleY = H - strip - Math.round(H * 0.02) - Math.round(H * 0.19);
      if (card.subtitle) {
        els.push({ type: 'chip', x: pad, y: titleY - Math.round(H * 0.075), size: Math.round(H * 0.038), bg: theme.accent, color: theme.accentText, text: card.subtitle, letterSpacing: 1.5 });
      }
      if (card.title) {
        els.push({ type: 'text', x: pad, y: titleY, w, h: Math.round(H * 0.19), text: (card.title || '').toUpperCase(), font: 'display', weight: 800, color: '#ffffff', align: 'left', valign: 'bottom', lineHeight: 1.0, letterSpacingEm: -0.01, autofit: { min: Math.round(H * 0.05), max: Math.round(H * 0.09), lines: 2 } });
      }
    }
    if (card.date) {
