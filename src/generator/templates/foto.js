...
module.exports = {
  ...
  build(card, ctx) {
    ...
    if (card.date) {
      els.push({ type: 'text', x: Math.round(W * 0.5), y: H - strip + Math.round(H * 0.02), w: Math.round(W * 0.5) - pad, h: Math.round(H * 0.05), text: card.date.toUpperCase(), font: 'text', weight: 700, color: '#ffffff', align: 'right', valign: 'center', size: Math.round(H * 0.028) });
    }
