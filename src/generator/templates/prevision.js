...
const { iconSvg, keyOf } = require('./clima');
...
module.exports = {
  ...
  build(card, ctx) {
    ...
    days.forEach((day, i) => {
      ...
      const c = first ? theme.accent : theme.text;
      if (i > 0) {
      ...
      els.push({ type: 'svg', anim: 'float', x: Math.round(x + (colW - icoS) / 2), y: Math.round(H * 0.29), w: icoS, h: icoS, svg: iconSvg(keyOf(day.cond), c) });
      // Máxima gigante
