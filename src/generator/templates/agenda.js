...
module.exports = {
  ...
  build(card, ctx) {
    ...
    els.push({ type: 'text', x: pad, y: 0, w: Math.round(W * 0.6), h: topH, text: (card.title || 'AGENDA').toUpperCase(), font: 'display', weight: 800, color: accText, align: 'left', valign: 'center', size: Math.round(topH * 0.42), letterSpacingEm: 0.03 });
    ...
    items.forEach((it, i) => {
      ...
      els.push({ type: 'text', x: nameX, y: Math.round(cyy - rowH * 0.28), w: nameW, h: Math.round(rowH * 0.42), text: it.name, font: 'display', weight: 800, color: rowText, align: 'left', valign: hasV ? 'bottom' : 'center', autofit: { min: Math.round(H * 0.035), max: Math.round(rowH * 0.42), lines: 1 } });
