...
module.exports = {
  ...
  build(card, ctx) {
    ...
    const acc = '#D6FF00', accText = '#0E0E0E', dark = '#0E0E0E';
    ...
    els.push({ type: 'text', x: pad, y: 0, w: Math.round(W * 0.6), h: topH, text: (card.title || 'AGENDA').toUpperCase(), font: 'display', weight: 800, color: dark, align: 'left', valign: 'center', size: Math.round(topH * 0.42), letterSpacingEm: 0.03 });
    ...
    const items = String(card.body || '').split(/\r?\n/).map((s) => s.trim()).filter(Boolean).slice(0, 3)
      .map((l) => { const p = l.split('|').map((x) => x.trim()); return { time: p[0] || '', name: (p[1] || p[0] || '').toUpperCase(), venue: (p[2] || '').toUpperCase() }; });
    ...
    const nameX = pad + boxW + Math.round(W * 0.025), nameW = W - nameX - pad;
    items.forEach((it, i) => {
      ...
      els.push({ type: 'text', x: nameX, y: Math.round(cyy - rowH * 0.28), w: nameW, h: Math.round(rowH * 0.42), text: it.name, font: 'display', weight: 800, color: dark, align: 'left', valign: hasV ? 'bottom' : 'center', autofit: { min: Math.round(H * 0.035), max: Math.round(rowH * 0.42), lines: 1 } });
