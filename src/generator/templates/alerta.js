...
module.exports = {
  ...
  build(card, ctx) {
    ...
    const detailH = card.body ? Math.round(H * 0.09) : 0;
    ...
    if (card.body) {
      els.push({ type: 'text', x: pad, y: zoneTop + titleH + Math.round(H * 0.01), w: Math.round(W * 0.68), h: detailH, text: card.body, font: 'text', weight: 800, color: theme.text, align: 'left', valign: 'center', size: Math.round(H * 0.052), lineHeight: 1.12 });
    }
    if (card.date) {
