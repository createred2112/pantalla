...
// Identificador estable del único diseño vivo. Forma parte de layouts y caché.
...
function airBodyElement(card, ctx, band, base = null) {
  ...
  const fallback = {
    ...
    valign: 'center',
    lineHeight: 1,
    autofit: { min: Math.round(H * 0.055), max: Math.round(H * 0.075), lines: 1 },
  };
  const next = {
    ...fallback,
    ...(base || {}),
...
function repairAirFrame(card, ctx, frame, opts = {}) {
  ...
  const rule = {
    ...
    w: Math.round(W * 0.28),
    h: Math.max(8, Math.round(H * 0.014)),
    color: theme.accent,
    colorTheme: 'accent',
    radius: 4,
  };
...
function weatherIconElement(card, ctx) {
  ...
  const clima = require('./templates/clima');
  const pad = Math.round(W * 0.05);
  const zoneY = Math.round(H * 0.16);
  const zoneH = Math.round(H * 0.46);
  const icoS = Math.round(Math.min(zoneH, W * 0.32));
  return {
...
function weatherBandTextElement(card, ctx, band, base = null) {
  ...
  const fallback = {
    ...
    lineHeight: 1,
    letterSpacingEm: 0,
    autofit: { min: Math.round(H * 0.06), max: Math.round(H * 0.105), lines: 1 },
  };
  const next = {
    ...fallback,
    ...(base || {}),
...
function repairWeatherFrame(card, ctx, frame, opts = {}) {
  ...
  if (summary) {
    ...
    if (!band) {
      ...
      band.x = 0;
      band.w = W;
      band.h = Math.max(Number(band.h) || 0, Math.round(H * 0.17));
...
// Los layouts creados con el diseño actual llevan design:'v2'. Los antiguos
// quedan guardados en los datos, pero no se aplican sobre el diseño definitivo.
function cardLayoutFor(card) {
  if (!card.layout || !Array.isArray(card.layout.elements)) return null;
  return card.layout.design === designVersion() ? card.layout : null;
}
